...
const CheckoutPage = () => {
    ...
    //by sing useSelector hook we get the cartitems
    const cartItems=useSelector(state=>state.cart.cartItems);
    const totalPrice=cartItems.reduce((acc,item)=> acc +item.newPrice,0).toFixed(2)
    const currentUser=true;//TODO get user from auth
    const [isChecked,setIsChecked]=useState(false)
    
    //useForm hook it justs takes input from user through form
    const {
            register,
            handleSubmit,
            watch,
    ...
    const onSubmit = async (data) =>{ 
        ...
        const updatedData = {
        ...data,
        items: cartItems.length, // Replace with dynamic values as needed
      ...
      Swal.fire({
        title: "Order Placed!",
        text: "Item will be delivered to your address",
        icon: "success",
                            ...
                            <div className="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                                ...
                                <div className="md:col-span-5 mt-3">
                                    <div className="inline-flex items-center">
                                        <input 
                                        {...register("billing_same", { required: "Name is required" })}
